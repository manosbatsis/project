      ...
      <el-table-column label="上架计提金额" align="center" min-width="130">
        <template slot-scope="scope">
          <span>
            返利:
            {{
              scope.row.shelfRebateAmount
                ? scope.row.currency + ' ' + scope.row.shelfRebateAmount
                : '0'
            }}
          </span>
        </template>
      ...
      <el-table-column label="已核销暂估金额" align="center" min-width="130">
        <template slot-scope="scope">
          <span>
            返利:
            {{
              scope.row.verifyRebateAmount
                ? scope.row.currency + ' ' + scope.row.verifyRebateAmount
                : '0'
            }}
          </span>
        </template>
      ...
      >
        <template slot-scope="scope">
          <span>
            返利:
            {{
              scope.row.nonVerifyRebateAmount
                ? scope.row.currency + ' ' + scope.row.nonVerifyRebateAmount
                : '0'
            }}
          </span>
        </template>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="90">
        <template slot-scope="scope">
          <el-button
            ...
            @click="
              linkTo(
                `/receivemoney/toBWriteoffDetail?id=${scope.row.id}&type=fee`
              )
            "
    ...
    methods: {
      ...
      async exportExcel(type) {
        ...
        if (type === 'detail') {
          ...
          this.$exportFile(exportToBTempRebateDetailsUrl, opt)
        }
        console.log(opt)
      ...
      async refresh(row) {
        this.tableData.loading = true
        try {
          await flashTobTemporaryReceiveBill({ id: row.id, type: 1 })
      ...
      async doneHeXiao() {
        ...
        try {
          ...
          await endTobTemporaryReceiveBill({ tobIds, type: 1 })
