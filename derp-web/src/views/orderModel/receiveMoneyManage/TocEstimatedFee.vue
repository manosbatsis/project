<template>
  ...
  <div>
    ...
    >
      ...
      <template slot="adjustmentRmbAmount" slot-scope="{ row }">
        {{ `${row.currency} ${row.adjustmentRmbAmount || 0}` }}
      </template>
      ...
      <template slot="action" slot-scope="{ row }">
        <el-button
          ...
          @click="
            linkTo(`/receivemoney/tocestimateddetail?id=${row.id}&type=fee`)
          "
        >
          详情
        </el-button>
...
<script>
  ...
  import {
    listToCTempCostReceiveBill,
    refreshBill,
    // getBillCount,
    exportCostBillUrl,
    exportCostSummaryOrderUrl,
    exportTempCostBillUrl
  } from '@a/reportManage'
    ...
    methods: {
      ...
      async getList(pageNum) {
        ...
        try {
          ...
          const { data } = await listToCTempCostReceiveBill({
      ...
      async exportList() {
        ...
        try {
          if (this.tableChoseList.length) {
            ...
            const {
              data: { code }
            } = await exportCostBillUrl({ ...this.searchProps })
            code === '00' && this.$alertSuccess('请在任务列表查看进度')
          }
        } catch (error) {
          this.$errorMsg(error.message)
      ...
      async exportSummaryOrder() {
        ...
        try {
          if (this.tableChoseList.length) {
            ...
            const {
              data: { code }
            } = await exportTempCostBillUrl({ ...this.searchProps })
            code === '00' && this.$alertSuccess('请在任务列表查看进度')
          }
        } catch (error) {
          this.$errorMsg(error.message)
      ...
      async exportTempBill() {
        ...
        try {
          if (this.tableChoseList.length) {
            this.$showToast('确定导出勾选数据？', async () => {
              const ids = this.tableChoseList.map((item) => item.id).toString()
              await exportCostSummaryOrderUrl({ ids })
            ...
            await exportCostSummaryOrderUrl({ ...this.searchProps })
            this.$alertSuccess('请在任务列表查看进度')
          }
        } catch (error) {
          this.$errorMsg(error.message)
