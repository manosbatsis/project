<template>
  ...
  <div class="page-bx bgc-w">
    ...
    <el-row class="mr-b-10 mr-t-20">
      <el-button id="sale-delivery-all" type="primary" @click="allDaTuo">
        整批打托
      </el-button>
    </el-row>
    <JFX-table
    ...
    >
      ...
      <el-table-column
        ...
        show-overflow-tooltip
      ></el-table-column>
      <el-table-column align="center" label="出库好品量" width="120">
      ...
      <el-table-column align="center" label="出库坏品量" width="120">
        <template slot="header">
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          returnOutDate: [
            { requsred: true, message: '请选择', trigger: 'change' }
          ]
        },
        action: '',
        saveLoading: false
      ...
      async getList() {
        try {
          ...
          const {
            data,
            data: { itemList }
          } = await toOutDepotReport({ id: this.$route.query.id })
        ...
        const { isInventoryValidate } = await this.$checkInventoryNum({
          itemList: this.tableData.list.map((item) => {
            return {
              ...
              badNum: item.badGoodsNum || 0,
              batchNo: item.returnBatchNo || '',
              tallyingUnit: this.saleReturnOrder.tallyingUnit || '',
