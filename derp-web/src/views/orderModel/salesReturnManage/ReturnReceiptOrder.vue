      ...
      <JFX-Form :model="searchProps" ref="searchForm">
        <el-row :gutter="10">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6">
            ...
            >
              <el-select
                ...
                clearable
                filterable
                :data-list="getSelectList('saleReturnIdepot_statusList')"
              >
                <el-option
                  v-for="item in selectOpt.saleReturnIdepot_statusList"
                  :key="item.key"
                  :label="item.value"
                  :value="item.key"
                />
          ...
          <el-col :xs="24" :sm="18" :md="18" :lg="12" :xl="12">
            <el-form-item prop="date" label="退货入库时间：">
              <el-date-picker
                clearable
                v-model="date"
                type="datetimerange"
    ...
    <el-row class="mr-t-20 mr-b-20">
      <el-col :span="24">
        <el-button
          type="primary"
          @click="exportList"
          v-permission="'saleReturnIdepot_exportSaleReturnIdepot'"
        >
          导出
        </el-button>
        <el-button
          type="primary"
          @click="importFile"
          v-permission="'saleReturnIdepot_importSaleReturnIdepot'"
        >
          导入
        </el-button>
        <el-button
          type="primary"
          @click="examineOrder"
          v-permission="'saleReturnIdepot_examineSaleReturnIdepot'"
        >
          审核
        </el-button>
        <el-button
          type="primary"
          @click="generateSaleSdOrder"
          v-permission="'saleReturnIdepot_generateSaleSdOrder'"
        >
          生成销售SD单
        </el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column align="center" width="120" label="操作" fixed="right">
        <template slot-scope="{ row }">
          <el-button
            type="text"
            @click="linkTo(`/sales/returnreceiptdetail?id=${row.id}`)"
            v-permission="'saleReturnIdepot_details'"
          >
            详情
          </el-button>
          <el-button
...
<script>
  ...
  import {
    getSaleReturnIdepotList,
    exportSaleReturnIdepotUrl,
    getSaleReturnIdepotrCount,
    importSaleReturnIdepotUrl,
    auditSaleReturnIdepot,
    generateSaleSdOrder
  } from '@a/salesReturnManage/returnReceiptOrder.api'
  export default {
    mixins: [commomMix],
    data() {
      return {
    ...
    methods: {
      ...
      async getList(pageNum) {
        // 退货入库时间
        this.searchProps.returnInDateStartDate =
          this.date && this.date.length ? this.date[0] : ''
        this.searchProps.returnInDateEndDate =
          this.date && this.date.length ? this.date[1] : ''
        try {
          this.tableData.pageNum = pageNum || this.tableData.pageNum
          this.tableData.loading = true
          const { data } = await getSaleReturnIdepotList({
        ...
        try {
          if (this.tableChoseList.length) {
            this.$showToast('确定导出勾选数据？', async () => {
              ...
              this.$exportFile(exportSaleReturnIdepotUrl, { ids })
            })
          } else {
            const { data } = await getSaleReturnIdepotrCount({
              ...this.searchProps
            })
            if (data > 10000) {
              return this.$errorMsg('导出的数量过大，请填写筛选条件再导出')
            }
            this.$exportFile(exportSaleReturnIdepotUrl, { ...this.searchProps })
          }
        } catch (error) {
      ...
      examineOrder() {
        ...
        this.$showToast('确定审核当前项？', async () => {
          const ids = this.tableChoseList.map((item) => item.id).toString()
          try {
            const { data } = await auditSaleReturnIdepot({ ids })
            if (!data.failure) {
