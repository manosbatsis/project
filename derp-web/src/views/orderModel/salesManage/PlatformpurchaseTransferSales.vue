<template>
  <!-- 转销售订单页面 -->
  <div class="page-bx bgc-w edit-bx" :loading="viewLoading">
    ...
    <div class="mr-t-30 flex-c-c">
      <el-button type="primary" @click="comfirm" :loading="comfirmLoading">
        确 定
      </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      async init() {
        const { ids, currency, customerId } = this.$route.query
        // 币种
      ...
      async reloadPrice() {
        ...
        // 存在商品的情况下继续
        if (!this.tableData.list.filter((item) => item.saleGoodsId).length) {
          return false
        }
