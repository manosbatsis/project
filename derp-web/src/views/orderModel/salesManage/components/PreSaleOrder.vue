  ...
  export default {
    ...
    methods: {
      ...
      comfirm() {
        ...
        let flag = true
        const goodsIdArr = this.tableChoseList.map((item) => item.id)
        const hash = {}
        const temp = []
        ...
        // const uniqueId = this.tableChoseList.map(item => item.id)
        ...
        // const checkList = this.tableData.list.filter(item => uniqueId.includes(item.id))
        ...
        // for (let i = 0; i < this.tableData.list; i++) {
        //   const { staySaleNum, num } = this.tableData.list[i]
        //   if (num === null || num === undefined) {
        //     console.log(1)
        //     this.$errorMsg(`表格第${i + 1}行，本次销售量不能为空`)
        //     flag = false
        //     break
        //   }
        //   if (num > staySaleNum) {
        //     this.$errorMsg(`表格第${i + 1}行，本次销售量不得大于待销量`)
        //     flag = false
        //     break
        //   }
        // }
        ...
        const target = this.selectOpt.customer_list.find(
          (item) => item.key === this.ruleForm.customerId.toString()
        )
        const merchantName = target ? target.value : ''
        const params = {
          ...this.ruleForm,
          ids,
          type: '1',
        ...
        sessionStorage.setItem('persaleObj', json)
        this.linkTo('/sales/salesorderadd?source=1')
        this.$emit('comfirm')
      },
