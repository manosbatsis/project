  ...
  <div class="page-bx edit-bx bgc-w">
    ...
    >
      <el-row :gutter="10">
        ...
        <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6" v-if="!isEdit">
          <el-form-item label="供应商：" prop="customerId">
            <el-select
              ...
              placeholder="请选择"
              :data-list="
                getCusOrSurpSelectBean('supplier_list', { typeFlag: '6' })
              "
              filterable
              clearable
            >
        ...
        <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6" v-if="isEdit">
          <el-form-item label="供应商编码：" prop="customerCode">
        ...
        <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6">
          <el-form-item label="采购价：" prop="supplyPrice">
            <JFX-Input
              v-model="baseForm.supplyPrice"
              :min="0"
              :precision="8"
              placeholder="请输入采购价"
              clearable
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          supplyPrice: { required: true, message: '请输入采购价' },
          ...
          stockLocationTypeId: { required: false, message: '请完善数据' }
        },
        // 上传编码
        uploadCode: '',
        // 上传地址
    ...
    methods: {
      ...
      async addInit() {
        ...
        /* 校验库位类型 */
        this.validateStockLocationType()
      },
      // 编辑初始化
      async editInit() {
        this.isEdit = true
        ...
        // 判断有没有code,用于附件上传
        if (!uploadCode) {
          const { data: code } = await getCode({ id })
          uploadCode = code
        }
        ...
        this.baseForm.supplyPrice = data.supplyPriceLabel
        /* 校验库位类型 */
        this.validateStockLocationType()
        /* 获取库位类型下拉列表 */
      ...
      handleSubmit() {
        this.$refs.baseForm.validate(async (valid) => {
          ...
          try {
            ...
            if (this.isEdit) {
              // 编辑
              await modifySMPrice(params)
            } else {
              // 新增 复制
              await addSMPrice(params)
            }
      ...
      /* 校验库位类型 */
      async validateStockLocationType() {
        ...
        try {
          ...
          /* 当公司不为卓烨，事业部不为母婴美赞臣组时库位类型需清空并禁用该字段 */
          !locationManage && (this.baseForm.stockLocationTypeId = '')
        } catch (error) {
          this.$errorMsg(error.message || '系统异常')
        }
      }
