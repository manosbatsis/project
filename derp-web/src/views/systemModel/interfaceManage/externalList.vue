  ...
  <div class="page-bx bgc-w">
    ...
    <el-row class="mr-t-20 mr-b-20">
      <el-col :span="24">
        <el-button
          type="primary"
          v-permission="'apiExternal_toAddPage'"
          @click="linkTo(`/interface/addExternal`)"
        >
          新增
        </el-button>
        <el-button
          type="primary"
          v-permission="'apiExternal_delApiExternal'"
          @click="removeItem"
        >
          删除
        </el-button>
    ...
    >
      <template slot="action" slot-scope="{ row }">
        <el-button
          type="text"
          v-if="row.status === '1'"
          v-permission="'apiExternal_auditApiExternal'"
        ...
        <el-button
          type="text"
          v-permission="'apiExternal_toEditPage'"
          @click="
            linkTo(`/interface/addExternal?id=${row.id}`, '对外接口配置编辑')
          "
          style="padding-left: 6px"
        >
          编辑
    ...
    methods: {
      ...
      auditItem(id, status) {
        this.$showToast('确定启用/禁用选中对象？', async () => {
          await auditApiExternal({ id, status })
          this.$successMsg('操作成功')
          this.getList()
        })
      }
