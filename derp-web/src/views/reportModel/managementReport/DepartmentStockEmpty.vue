  ...
  <div class="page-bx bgc-w">
    ...
    <el-row class="mr-t-10">
      <el-col :span="17">
        <el-button
          type="primary"
          v-permission="'DepartmentStockEmptyExport'"
          @click="exportTalbe"
        >
          导出
        </el-button>
    ...
    <el-row v-loading="tableData.loading">
      <el-col :span="21">
        ...
        >
          ...
          <template slot="inventoryNum" slot-scope="{ row }">
            {{ row.inventoryNum ? numberFormat(row.inventoryNum) : 0 }}
          </template>
          <template slot="avgSaleNumOfNinety" slot-scope="{ row }">
            {{
              row.avgSaleNumOfNinety ? numberFormat(row.avgSaleNumOfNinety) : 0
            }}
          </template>
          <template slot="inventoryCleanDay" slot-scope="{ row }">
            {{
              row.inventoryCleanDay !== null
                ? numberFormat(row.inventoryCleanDay)
                : '-'
            }}
          </template>
        </JFX-table>
...
<script>
  ...
  import {
    getDepartmentInventoryCleanStatistic,
    exportDepartmentInventoryCleanStatistic,
    getMerchantList,
  ...
  export default {
    ...
    methods: {
      ...
      async getList() {
        ...
        const { data } = await getDepartmentInventoryCleanStatistic(params)
      ...
      exportTalbe() {
        ...
        console.log(params)
        this.$exportFile(exportDepartmentInventoryCleanStatistic, { ...params })
      },
      // 重置搜索表单
      resetSearchForm() {
