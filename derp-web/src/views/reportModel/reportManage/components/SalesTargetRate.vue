  ...
  <div class="page-bx bgc-w" style="padding-top: 0">
    ...
    >
      ...
      <el-table-column
        ...
        label="标准品牌"
      />
      <el-table-column align="center" min-width="120" label="To B销量 / 完成率">
        <template slot-scope="{ row }">
          {{ `${row.toBNum} / ${(row.toBRate * 100).toFixed(2)}%` }}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="120" label="To C销量 / 完成率">
        <template slot-scope="{ row }">
          {{ `${row.toCNum} / ${(row.toCRate * 100).toFixed(2)}%` }}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="120" label="总销量 / 总完成率">
        <template slot-scope="{ row }">
          {{ `${row.num} / ${(row.rate * 100).toFixed(2)}%` }}
        </template>
      </el-table-column>
    </JFX-table>
    <!-- 表格 end-->
