...
<script type="text/javascript">
            ...
            save:function(){
            	...
            	if(ids!=""){
    	            ...
    	            $.each(resultJson,function(index,event){
    	            	...
    	            	var maxIndex = getGoodMaxIndex();
                        maxIndex = parseInt(maxIndex) + index;

                        $tr+="<tr><td style='text-align:center'><input type='checkbox' name='item-checkbox'><input type='hidden' name='goodsId' value='"+event.id+"'/></td>"+
                        "<td style='text-align:center'><input type='text' class='good-seq' name='seq' style='width:70px;text-align:center;' value='"+ maxIndex +"'/></td>"+
                        ...
                        '<td><input type="hidden"  name="grossWeight-hidden" value="'+(event.grossWeight==null?'0':event.grossWeight)+'"><input type="text" style=\'width:60px;text-align:center;\'  class="form-control" id="goods-rough" name="grossWeight" value="'+(event.grossWeight==null?'0':parseFloat(event.grossWeight).toFixed(5))+'" onkeyup="numFormat(this,5)" onchange="calculateWeight5()" maxlength="11"></td>'+
                        '<td><input type="hidden"  name="netWeight-hidden" value="'+(event.netWeight==null?'0':event.netWeight)+'"><input type="text" style=\'width:60px;text-align:center;\'  class="form-control" id="goods-suttle" name="netWeight" value="'+(event.netWeight==null?'0':parseFloat(event.netWeight).toFixed(5))+'" onkeyup="numFormat(this,5)" onchange="suttleWeight5()" maxlength="11"></td>'+
    ...
    //计算净重之和
    function suttleWeight5() {
        var totalGrossWeight = 0;
        $("#table-list tbody tr").each(function (index, item) {
            var grossWeight = $(this).find("td").eq(15).children('input[name="netWeight"]').val();//毛重  yigai
            if (!grossWeight) {
                grossWeight = 0;
            }
            totalGrossWeight += parseFloat(grossWeight);
