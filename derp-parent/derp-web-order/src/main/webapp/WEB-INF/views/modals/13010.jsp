...
<div class="row">
    <div class="col-12">
        <div>
            ...
            <div id="invoice-modal"  class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;left: -200px; top: 100px;">
                <div class="modal-dialog">
                    <div class="modal-content" style="width: 800px;text-align: center;">
                        <div class="modal-header">
                            <button type="button" class="close" onclick="$m13010.hide()">×</button>
                                ...
                                <div class="col-12">
                                    <div class="row">
                                        ...
                                        <div class="col-8">
                                            ...
                                            <button type="button" class="btn btn-light waves-effect waves-light btn_default btn-sm" onclick='$m13010.hide();'>取消</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    var $m13010={
        init:function(id, type){
            var relIds = [];
            $m13010.params.id = id;
            $m13010.params.type = type;
            $("#synNc-tbody").empty();
            //查询关联应收账单
            $custom.request.ajax($m13010.params.listReceiveBillUrl, {"id":id, "dataSource":"2"}, function(result){
                if(result.state == 200) {
                    ...
                    $m13010.params.relIds = uniqueArray(relIds).join(",");
                }
            });
            $($m13010.params.modalId).modal({backdrop: 'static', keyboard: false});
        },
        hide:function(){
            $($m13010.params.modalId).modal("hide");
            if ($m13010.params.type == '1') {
                setTimeout(function () {
    ...
    function synNC() {
        $custom.request.ajax($m13010.params.synNCUrl, {"id":$m13010.params.id, "dataSource":"2"}, function(res){
            if(res.state == 200) {
                if (res.data.code == '00') {
    ...
    function exportBillInvoice() {
        //导出
        var url = serverAddr+"/receiveBillInvoice/download.asyn?ids="+$m13010.params.relIds;
