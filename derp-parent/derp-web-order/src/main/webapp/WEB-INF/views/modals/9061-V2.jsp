							...
							<div class="form-row mt20 ml15 table-responsive">
								<table id="purchaseTable-list" class="table table-striped" style="width: 100%;">
									...
									<tr>
										...
										<td></td>
										<td id="purchaseTotalAmount" style="text-align: center;"></td>
									</tr>
								</table>
							</div>
...
var $m9016V2 = {
        ...
        show: function(data){
            ...
            for (var i = 0; i < data.itemList.length; i++) {
                ...
                var num = m.num;
                var price = m.price;
                var amount = parseFloat(m.amount).toFixed(2);
                html += '<tr>'
            ...
            $($m9016V2.params.modalId).modal("show");
        },
        hide:function(){
            $($m9016V2.params.modalId).modal("hide");
        },
        params:{
    ...
    function chooseSave(){
       ...
       if(modalPoNo !="" && modalPoNo != null && modalPoNo != undefined){
    	   $custom.request.ajax(serverAddr+"/preSale/checkExistPurchaseByPO.asyn",{"poNo":modalPoNo},function(result){
	 ...
	 function saveData(){
        ...
        form['json']=jsonStr;
		 ...
		 $custom.request.ajax(serverAddr+"/preSale/GeneratePurchaseOrder.asyn",form,function(result){
            if(result.state==200){
            	if(result.data == null){
    ...
    function getSalePoNo(){
    	var poNo = $m9016V2.params.paramJson.poNo;
