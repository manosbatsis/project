...
<div class="row">
    <div class="col-12">
        <div>
            ...
            <div id="verify-modal"  class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
                            ...
                            <div class="form-row m-t-50">
                                <div class="col-12">
                                    <div class="row">
                                        <div class="col-4"></div>
                                        <div class="col-2">
                                            <button type="button" class="btn btn-info waves-effect waves-light fr btn-sm" onclick='$m13020.save()'>确认</button>
                                        </div>
                                        <div class="col-2">
                                            <button type="button" class="btn btn-light waves-effect waves-light btn_default btn-sm" onclick='$m13020.hide();'>取消</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    var $m13020={
        ...
        init:function(id, code, receivablePrice, customerName, currency){
            $derpTimer.lessThanNowDateTimer("#receiveDate", "yyyy-MM-dd") ;
            //初始化plupload
            $m13020.params.uploader = new plupload.Uploader({ //实例化一个plupload上传对象
            ...
            $m13020.params.uploader.init(); //初始化

            $m13020.params.id = id;
            $m13020.params.currency = currency;
            $("#code").html(code);
            ...
            //查询核销记录
            $custom.request.ajax($m13020.params.verifyItemUrl, {"id": id}, function (result) {
                if (result.state == 200) {
                    var totalRmbAmount = result.data.totalRmbAmount;
                    $m13020.params.receivablePrice = totalRmbAmount;
                    $("#receivablePrice").html(totalRmbAmount);
                    var list = result.data.verifyItemModels;
                    if (list) {
                        var html = "";
                        ...
                        $m13020.params.existPrice = totalPrice;
                        if (Math.abs(totalPrice) < Math.abs(totalRmbAmount)) {
                            html += '<tr>'
            ...
            $($m13020.params.modalId).modal({backdrop: 'static', keyboard: false});
        },
        hide:function(){
            $($m13020.params.modalId).modal("hide");
        },
        save:function () {
            ...
            if ((Math.abs(Number(price) + Number($m13020.params.existPrice))).toFixed(2)> Math.abs($m13020.params.receivablePrice)) {
            ...
            //上传文件
            var files = $m13020.params.uploader.files ;
            ...
            $m13020.params.uploader.start() ;
            var json = {"billId":$m13020.params.id, "receiceNo":receiceNo,"receiveDateStr":receiveDateStr, "price":price}
            $custom.request.ajax($m13020.params.saveUrl, json , function(res){
        ...
        params: {
            verifyItemUrl: serverAddr + '/toCReceiveBill/getVerifyItems.asyn?r=' + Math.random(),
            saveUrl: serverAddr + '/toCReceiveBill/saveReceiveBillVerify.asyn?r=' + Math.random(),
            modalId: '#verify-modal',
            id: null,
            existPrice: 0,
            currency: null,
            uploader: null,
    ...
    function removeFile(fileId){
        ...
        $m13020.params.uploader.removeFile(fileId);
    }
</script>