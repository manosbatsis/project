                                    ...
                                    <div class="info_item">
                                        <div class="info_text">
                                            ...
                                            <input type="text" class="edit_input form_datetime date-input"
                                                   name="transferInDate" id="transferInDate" />
                                        </div>
                                        <div class="info_text">
                                ...
                                <table id="table-list" class="table table-striped" >
                                    ...
                                    <c:forEach items="${orderItem }" var="item">
                                        <tr>
                                            ...
                                            <td>${item.inBarcode}<input type="hidden" name="barcode"
                                                                      value="${item.inBarcode}"/></td>
                                            <td>
                                                <input type="hidden" name="inUnit" value="${item.inUnit}"/>
                                                <c:if test="${item.inUnit eq '00' }">托盘</c:if>
                                                <c:if test="${item.inUnit eq '01' }">箱</c:if>
                                                <c:if test="${item.inUnit eq '02' }">件</c:if>
                                            </td>
                                            <td>${item.inTransferNum}<input type="hidden" name='inTransferNum'
                                                                  style='width:70px;text-align:center;'
                                                                  class='goods-num' value="${item.inTransferNum}"></td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#allShelf").click(function () {
            ...
            $('#table-list tbody tr').each(function (i) {
                if ($(this).find('input[name="item-checkbox"]').is(":checked")) {
                    $(this).find('input[name="transferNum"]').val($(this).find('input[name="inTransferNum"]').val());
        ...
        $("#save-buttons").click(function () {
            var inDepotBatchValidation = '${transfer.inDepotBatchValidation}';
            var transferInDate = $('#transferInDate').val();
            if (!transferInDate) {
            ...
            var check = true;
            //入库时间限制
            var inDepotUrl = serverAddr+"/transfer/validInDepotDate.asyn";
            $custom.request.ajax(inDepotUrl,{"id":id, "transferInDate":transferInDate},function(result){
                if (result.state == '200' ) {
                    ...
                    if (check) {
                        ...
                        $custom.request.ajax(validUrl,{"id":id, "orderType":1},function(res){
                            if (res.state == '200') {
                                if(res.data.code!='00'){
                                    $custom.alert.warningText(res.data.message);
                                    return;
                                } else {
                                    var goodsNos = [];
                                    var goodsList = [];
                                    $('#table-list tbody tr').each(function (i) {// 遍历 tr
                                        ...
                                        var inTransferNum = $(this).find('input[name="inTransferNum"]').val();
                                        ...
                                        if (inDepotBatchValidation != '0') {
                                            if (!batchNo) {
                                                check = false;
                                                $custom.alert.error("入库仓库为批次效期强校验或入库强校验，批次不能为空");
                                                return;
                                            }

                                            if (!productionDate) {
                                                check = false;
                                                $custom.alert.error("入库仓库为批次效期强校验或入库强校验，生产日期不能为空");
                                                return;
                                            }

                                            if (!overdueDate) {
                                                check = false;
                                                $custom.alert.error("入库仓库为批次效期强校验或入库强校验，失效日期不能为空");
                                                return;
                                            }
                                        }

                                        var totalNum = parseInt(transferNum) + parseInt(wornNum);
                                        if (totalNum != parseInt(inTransferNum)) {
                                    ...
                                    if (check) {
                                        var json = {};
                                        json.transferInDate = transferInDate;
                                        ...
                                        if (goodsNos.length > 0) {
                                            $custom.alert.warning("货号:" + goodsNos.join(',') +  "上架总量不等于计划调入量，请确认是否提交",function(){
                                                var countUrl = serverAddr+"/transferIn/saveTransferInDepot.asyn";
