                ...
                <div>
                   <div class="col-12 row">
                    <div>
                       ...
                       <li class="breadcrumb-item"><a href="#">预售单新增</a></li>
                    </ol>
                    </div>
                   </div>
                    <!--  title   end  -->               
                    ...
                    <div class="edit_box">
                    	<div class="edit_item">
                            ...
                            <select class="edit_input" name="outDepotId" id="outDepotId" parsley-trigger="change" required reqMsg="出仓仓库不能为空">
                                <option value="">请选择</option>
                            </select>
                        </div>
                         <div class="edit_item">
                        ...
                        <div class="edit_item">
                            ...
                            <input type="text" class="edit_input" name="poNo" id="poNo"  placeholder="多PO输入时以&字符隔开" required reqMsg="PO单号不能为空">
                        </div>
                    </div>
                    <div class="edit_box">
                        <div class="edit_item">
                            <label class="edit_label"><i class="red">*</i>销售币种：</label>
                            <select class="edit_input" name="currency" id="currency" parsley-trigger="change" required reqMsg="销售币种不能为空">
                                <option value="">请选择</option>
                            </select>
                        </div>
                        <div class="edit_item" style="flex: 2">
                            <label class="edit_label"  style="flex: 0.369">备 注：</label>
                            <textarea rows="" cols="" class="edit_input" name="remark" id="remark" ></textarea>
                        </div>
                    </div>
                    <div class="edit_box">
                        <div class="edit_item" id="tallyingUnitDiv"></div>
                 ...
                 <!--  title   基本信息  start -->
                   ...
                   <thead>
                      ...
                      <tbody>
                    <%--	<tr><td><input type='checkbox' name='item-checkbox'><input type='hidden' name='goodsId' ></td>
                            <td><input type="hidden" name="goodsNo" /></td>
                            <td><input type="hidden" name="goodsName" /></td>
                            <td><input type="hidden" name="barcode" /></td>
                            <td><input type='text' name='num' style='width:70px;text-align:center;' class='goods-num'  onkeyup="value=value.replace(/[^\d]/g,'')"></td>
                            <td><input type='text' name='price' class='goods-price' style='width:70px;text-align:center;'></td>
                            <td><input type='text' name='amount'  class='goods-amount'  style='width:70px;text-align:center;' ></td>
                            <td><input type="hidden" name="brandName" /></td>
                        </tr>--%>
                      </tbody>
                  </table>                              
                   </div>
...
$module.constant.getConstantListByNameURL.call($('select[name="businessModel"]')[0],"preSaleOrder_businessModelList",null);
...
$module.depot.getSelectBeanByMerchantRel.call($('select[name="outDepotId"]')[0],null,{"type":"a,c"});
 $(document).ready(function() {
     ...
     $("#outDepotId").change(function() {
		...
		$("#save-buttons").click(function(){
			...
			var url = serverAddr+"/preSale/addPreSaleOrder.asyn";
			var form = {
			};
			var json = {};
			var customerId = $("#customerId").val();
			...
			var outDepotType = $("#outDepotId").find("option:selected").attr("type");
                $custom.request.ajaxSync("/depot/getDepotDetails.asyn", {"id":outDepotId}, function(result){
                    if (result.data.type=='c') {
                        ...
                        if (!tallyingUnit) {
                            $custom.alert.error("新增失败,出库仓库是海外仓，理货单位不能为空");
                            return;
                        }
                    }
                });
