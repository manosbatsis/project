...
<div class="content">
							...
							<div class="col-12 row">
								<div>
									<ol class="breadcrumb">
										...
										<li class="breadcrumb-item"><a
											href="javascript:dh_add();">编辑采购SD配置</a></li>
									</ol>
								</div>
							</div>
							<!--  title   end  -->
									...
									<div class="edit_item">
										<label class="edit_label"><i class="red">*</i> 生效时间：</label>
										 <input type="text" class="edit_input" name="effectiveTime" id="effectiveTime" style="font-size: 13px;" required
											reqMsg="生效时间不能为空" value="<fmt:formatDate value="${details.effectiveTime }" pattern="yyyy-MM-dd"/>">
									</div>
									<div class="edit_item">
										...
										<input type="text" class="edit_input" name="invalidTime" id="invalidTime" style="font-size: 13px;" required
											reqMsg="失效时间不能为空" value="<fmt:formatDate value="${details.invalidTime }" pattern="yyyy-MM-dd"/>">
									</div>
									...
									<tbody id="single-tbody">
									   <c:forEach items="${itemList }" var="item">
									       <c:if test="${ item.sdConfigSimpleType == '1'}">
									           <tr>
	                                               <td>
	                                                   <select class='edit_input singleSelect' onchange='getDetails(this, "1")' required reqMsg='SD类型不能为空'>
	                                                       <option value=''>请选择</option>
                                                           <c:forEach items="${singleList }" var="singleItem">
                                                               <option value='${singleItem.id }' <c:if test="${ singleItem.id == item.sdConfigId}">selected</c:if> >${singleItem.sdTypeName }</option>
                                                           </c:forEach>
	                                                   </select>
	                                               </td>
	                                               <td>${item.sdConfigSimpleName }</td>
	                                               <td><input class='edit_input proportion' type='text'required reqMsg='比例不能为空' value="${item.proportion }"></td>
	                                               <td><a href='#' onclick='itemDel(this);'>删除</a></td>
	                                           </tr>
									       </c:if>
									   </c:forEach>
									</tbody>
							...
							<div class="form-row mt20" style="padding: 0 10px;">
								...
								<table id="table-multi-list" class="table table-striped">
									...
									<tbody id="multi-tbody">
								   	   <c:set var="mutiNum" value="0"></c:set>
									   <c:forEach items="${itemList }" var="item">
										   <c:if test="${ item.sdConfigSimpleType == '2'}">
										   		<c:set var="mutiNum" value="${mutiNum + 1 }"></c:set>
	                                           <tr>
	                                           	   <td><input type='checkbox' class='muti-iCheck'></td>
	                                               <td>
	                                                   <select class='edit_input multiSelect' style='width: 80px;' onchange='getDetails(this, "2")' required reqMsg='SD类型不能为空'>
	                                                       <option value=''>请选择</option>
	                                                       <c:forEach items="${multiList }" var="multiItem">
	                                                           <option value='${multiItem.id }' <c:if test="${ multiItem.id == item.sdConfigId}">selected</c:if> >${multiItem.sdTypeName }</option>
	                                                       </c:forEach>
	                                                   </select>
	                                               </td>
	                                               <td>${item.sdConfigSimpleName }</td>
	                                               <td>${item.brandParent }</td>
	                                               <td><input type='hidden' class='commbarcode' value='${item.commbarcode }'>${item.commbarcode }</td>
	...
	//公司下拉
	$module.merchantAll.loadSelectData.call($('select[name="merchantId"]')[0], '${details.merchantId}');
	$module.businessUnit.getSelectBeanByMerchantDepotRel.call($("select[name='buId']")[0], '${details.buId}', '${details.merchantId}');
    $module.supplier.loadSelectDataByMerchantId.call($("select[name='supplierId']")[0], '${details.supplierId}', '${details.merchantId}');
	
	$derpTimer.init("#effectiveTime", "yyyy-MM-dd") ;
	$derpTimer.init("#invalidTime", "yyyy-MM-dd") ;
	
	$("#skus").html("合计SKU：${mutiNum}") ;
	...
	$(function(){
		...
		$("#merchantId").change(function(){
			...
			$module.businessUnit.getSelectBeanByMerchantDepotRel.call($("select[name='buId']")[0], '${details.buId}', merchantId);
			$module.supplier.loadSelectDataByMerchantId.call($("select[name='supplierId']")[0], '${details.supplierId}', merchantId);
		}) ;
