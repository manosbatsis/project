...
public class PdfPageHelper extends PdfPageEventHelper{
	...
	private void addWaterMar(PdfWriter writer) {
		...
		for(int i = 1 ; i <= 3; i ++) {
			...
			try {
				...
				waterMar.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED), 38);
				// 设置透明度
				waterMar.setGState(gs);
				// 设置水印对齐方式 水印内容 X坐标 Y坐标 旋转角度
				waterMar.showTextAligned(Element.ALIGN_RIGHT, waterMarContent, x, i * 250, 35);
				// 设置水印颜色
    ...
    public void onCloseDocument(PdfWriter writer, Document document) {
        ...
        total.showText(" 共"+String.valueOf(writer.getCurrentPageNumber() - 1)+"页");
        total.endText();
    }
