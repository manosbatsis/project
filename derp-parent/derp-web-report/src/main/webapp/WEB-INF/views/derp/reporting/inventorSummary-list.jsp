                    ...
                    <div class="card-box">
                       ...
                       <div class="row col-md-12 mt20">
                            <div class="button-list">
                                <shiro:hasPermission name="summary_createTask">
                                    <input type="button" class="btn btn-find waves-effect waves-light btn-sm" id="createTask" value="生成excel"/>
                                </shiro:hasPermission>

                                <shiro:hasPermission name="summary_flashInventorSummary">
                        ...
                        <div class="row mt20">
                            ...
                            <table id="datatable-buttons" class="table table-striped" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <th style="vertical-align: middle;min-width: 200px" rowspan="2">仓库</th>
                                    <th style="vertical-align: middle;min-width: 200px" rowspan="2">货品名称</th>
...
$(document).ready(function() {
   ...
   $("#selectTime").datetimepicker({
    ...
    //配置表格参数
    $mytable.params={
        url:serverAddr+'/summary/inventorSummaryList.asyn?r='+Math.random(),
        columns:[{  //序号
            data : null,
            bSortable : false,
            className : "",
            ...
            {data:'depotName'},{data:'productName'},{data:'brandName'},{data:'barcode'},{data:'factoryNo'},{data:'unitLabel'},{data:'supplyMinPrice'},{data:'monthBeginNum'},{data:'monthBeginNormalNum'},{data:'monthBeginDamagedNum'},
            { //操作
            	orderable: false,
            	width: "80px",
            	data: null,
            	render: function (data, type, row, meta) {
            	    return '<a href="javascript:details('+row.merchantId+','+row.depotId+',\''+row.ownMonth+'\',\''+row.barcode+'\',\'1\',\''+row.unit+'\')">'+row.monthInstorageNum+'</a>';
            	}
            }, 
            { //操作
            	orderable: false,
            	...
            	data: null,
            	render: function (data, type, row, meta) {
            	    return '<a href="javascript:details('+row.merchantId+','+row.depotId+',\''+row.ownMonth+'\',\''+row.barcode+'\',\'0\',\''+row.unit+'\')">'+row.monthOutstorageNum+'</a>';
            	}
            },
            {data:'monthInBadNum'},{data:'monthInBadNum'},{data:'profitlossGoodNum'},{data:'profitlossDamagedNum'},{data:'monthProfitlossNum'},{data:'monthEndNum'},{data:'monthEndNormalNum'},
            {data:'monthEndDamagedNum'},{data:'monthNormalExpireNum'},
            {
                orderable: false,
                sClass:"hiddenCol",
                data: 'availableNum'
            },
            {data:'monthSaleUnconfirmedNum'},{data:'monthPurchaseNotshelfNum'},{data:'monthTransferNotshelfNum'},{data:'monthEndAmount'},
    ...
    $mytable.rewriter.fnRowCallback=function(nRow, aData, iDisplayIndex){
    	...
    	/* if(aData.unit=='0'){
    		  $('td:eq(5)', nRow).html('托盘');
    	}else if(aData.unit=='1'){
  		  $('td:eq(5)', nRow).html('箱');
  	    }else if(aData.unit=='2'){
  		  $('td:eq(5)', nRow).html('件');
  	    }else{
  		  $('td:eq(5)', nRow).html(aData.unit);
  	    } */
    	//出库残损负号显示


        if (!${userType == '1' || userType == '2'}) {
            $('td:eq(16)', nRow).css("display","none");
        }
    };
...
//详情
function details(merchantId,depotId,ownMonth,barcode,operateType,unit) {
	$module.revertList.serializeListForm() ;
    $module.load.pageReport("act=120011&merchantId="+merchantId+"&depotId="+depotId+"&ownMonth="+ownMonth+"&barcode="+barcode
...
$("#flashReport").click(function(){
	...
	$custom.alert.warning("确定刷新仓库本月报表?",function(){
		var urlp = serverAddr+'/summary/flashInventorSummary.asyn';
		$custom.request.ajax(urlp,{"ownMonth":ownMonth,"flashForward":flashForward,"depotIds":depotIdsStr},function(result){
...
$("#createTask").on("click",function(){
	 ...
	 var depotNamesStr=depotNames.toString();
  	 //检查文件是否已生成
  	 var checkUrl = serverAddr+'/summary/createTask.asyn';; 
  	 $custom.request.ajax(checkUrl,{"depotIds":depotIdsStr,"depotNames":depotNamesStr,"ownMonth":ownMonth,"depotIdAlls":depotIdAllsStr,"depotNameAlls":depotNameAllsStr},function(result){
	     if(result.state==200&&result.data.code=='00'){
