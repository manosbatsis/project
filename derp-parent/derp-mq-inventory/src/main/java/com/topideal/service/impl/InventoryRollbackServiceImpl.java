...
public class InventoryRollbackServiceImpl implements InventoryRollbackService {
	...
	/* 打印日志 */
	private static final Logger LOGGER = LoggerFactory.getLogger(InventoryRollbackServiceImpl.class);

	@Autowired
	private InventoryDetailsDao inventoryDetailsDao;//收发明细	
	@Autowired
	...
	@Autowired
	private InventoryFreezeDetailsDao inventoryFreezeDetailsDao;// 冻结解冻
	


	@Override
	@SystemServiceLog(point = DERP_LOG_POINT.POINT_13201304900, model = DERP_LOG_POINT.POINT_13201304900_Label,keyword = "code")
	public void saveinventoryRollbackInfo(String json, String keys, String topics, String tags) throws Exception {
		LOGGER.info("库存数据回滚"+json);
		JSONObject jsonObject = JSONObject.fromObject(json);
		String code = jsonObject.getString("code");//出入库单号
		if (StringUtils.isBlank(code)) {
			throw new RuntimeException("库存数据回滚json 的code 不能为null");
		}
		//三、唯一单号校验
