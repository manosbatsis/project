...
public class UpdateGoodsInfoMQ extends ConsumerMQAbstract{
	...
	public UpdateGoodsInfoMQ(){
		super.setTags(MQOrderEnum.UPDATE_ORDER_GOODS_INFO.getTags());
		super.setTopics(MQOrderEnum.UPDATE_ORDER_GOODS_INFO.getTopic());
	}
	
	
	@Override
	public ConsumeConcurrentlyStatus execute(String json,String keys,String topics,String tags ) {
		LOGGER.info("===================order修改商品信息==================");
		try {
			updateGoodsInfoService.updateGoodsInfo(json,keys,topics,tags);
		} catch (Exception e) {
			LOGGER.error("order修改商品信息异常："+e.getMessage());
