...
@Service
public class FirstTallyServiceImpl implements FirstTallyService{
	/*打印日志*/
	private static final Logger LOGGER = LoggerFactory.getLogger(FirstTallyServiceImpl.class);
    ...
    @Override
    @SystemServiceLog(point="1201",model="初理货接口")
    public JSONObject tallyBaseInfo(String json,Long merchantId)throws Exception{
    	LOGGER.info("初理货Service 请求开始json:"+json);
    	// 实例化JSON对象
        ...
        // 根据电商企业查询商家
 		...
 		if (merchantInfoModel == null) {
			LOGGER.error("商家不存在,理货单号asn_no" + (String)jsonData.get("asn_no"));
			throw new RuntimeException("商家不存在,理货单号asn_no" + (String)jsonData.get("asn_no"));
		}
       ...
       // jsonMQData.put("palletNum", jsonData.get("panel_wallnumber"));//托版数量
        ...
        LOGGER.info("初理货Service 请求结束json:"+jsonMQData);
