 ...
 */
@Service
public class TransfersInStorageServiceImpl implements TransfersInStorageService {

	/* 打印日志 */
	private static final Logger LOGGER = LoggerFactory.getLogger(TransfersInStorageServiceImpl.class);
	...
	public JSONObject transfersInStorage(String json,Long merchantId,String isRookie) throws Exception {
		...
		jsonMQData.put("code", jsonData.getString("custtransfer_code"));// 调拨单号(例如调拨入库单单号)
																		// 必填
		jsonMQData.put("transferDate",  jsonData.getString("odate"));// 调拨时间
																		// 非必填
		// jsonMQData.put("orderCode",
		// jsonData.getString("customs_code"));//申报第海关必填
		// jsonMQData.put("orderCode", jsonData.getString("ciqb_code"));//申报地国检
		// 必填
		// jsonMQData.put("orderCode",
		// jsonData.getString("busi_scene"));//业务场景busi_type 必填
		// jsonMQData.put("orderCode", jsonData.getString("serve_types"));//
		// 服务类型必填
		// jsonMQData.put("orderCode", jsonData.getString("busi_type"));//业务类型
		// 必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("from_electric_code"));//调出电商企业编码 必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("from_electric_name"));// 调出电商企业名称 非必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("from_foundp_code"));//调出资金方 必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("to_electric_code"));// 调入电商企业编码 非必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("to_foundp_code"));// 调入资金方 必填
		// jsonMQData.put("orderCode", (String)jsonData.get("trust_code"));//
		// 委托单位 必填
		// jsonMQData.put("orderCode",
		// (String)jsonData.get("from_store_code"));// 调出仓库 必填
		// jsonMQData.put("orderCode", (String)jsonData.get("to_store_code"));//
		// 调入仓库 必填
		// jsonMQData.put("orderCode", (String)jsonData.get("contract_no"));//
		// 合同号 非必填
		...
		for (Object goods : jsonGoodsList) {
			...
			merchandiseInfoModel.setGoodsNo((String) goodsJSON.get("to_good_id"));
			...
			if (merchandiseInfoModel == null) {
				LOGGER.error("该app_key商家找不到该货号商品,商品货号" + (String) goodsJSON.get("to_good_id"));
				throw new RuntimeException("该app_key商家找不到该货号商品,商品货号" + (String) goodsJSON.get("to_good_id"));
			}
			// 判断标准条码是否为空
			...
			goodsMQJSON.put("num", goodsJSON.get("amount"));// 调拨数量 必填
			goodsMQJSON.put("batchNo", (String) goodsJSON.get("batch_id"));// 调入批次
																			// 非必填
			goodsMQJSON.put("productionDate", (String) goodsJSON.get("produce_d"));// 时间格式：yyyy-mm-dd
																					// HH:mi:ss
																					// 非必填
			goodsMQJSON.put("overdueDate", (String) goodsJSON.get("expire_date"));// 时间格式：yyyy-mm-dd
																					// HH:mi:ss
																					// 非必填
			goodsMQJSON.put("expiration", goodsJSON.get("expiration"));// 调入有效期天数
																		// 非必填
			String wornAmount = "0";
			...
			if (Integer.valueOf(goodsJSON.getString("amount")) != Integer.valueOf(salesAmount) +
				...
				LOGGER.error("商品货号:" + (String) goodsJSON.get("to_good_id") + "中批次号:" + goodsJSON.get("batch_id") + "的好品数量与坏品数量总和不等于批次总数量");
				throw new RuntimeException("商品货号:" + (String) goodsJSON.get("to_good_id") + "中批次号:" + goodsJSON.get("batch_id") + "的好品数量与坏品数量总和不等于批次总数量");
			}
