            ...
            <div class="col-12">
                ...
                <div class="col-12 row">
                    <div>
                       ...
                       <li class="breadcrumb-item "><a href="javascript:list();">客户列表</a></li>
                    </ol>
                    </div>
            </div>
                   <!--  过滤条件查询  start  -->
                    <form id="form1" >
                        <div class="form_box">
                            ...
                            <div class="form_item h35">
                                    <span class="msg_title">客户编号 :</span>
                                    <input class="input_msg" type="text" name="code" id="code"  >
                                    <span class="msg_title">客户名称 :</span>
                                    <!-- <select class="input_msg"  name="id" id="id" multiple="multiple" title="请选择" >
                              		</select> -->
                    ...
                    <div class="row col-md-12 mt20">
                        <div class="button-list">
                            <shiro:hasPermission name="customer_toAddPage">
                                <button type="button" class="btn btn-find waves-effect waves-light btn-sm" id="add-buttons">新增</button>
                            </shiro:hasPermission>
                            <shiro:hasPermission name="customer_delCustomer">
                                <button type="button" class="btn btn-find waves-effect waves-light btn-sm" onclick="$custom.request.delChecked('/customer/delCustomer.asyn');">删除</button>
                            </shiro:hasPermission>
                            <shiro:hasPermission name="customer_toImportPage">
                                <button type="button" class="btn btn-find waves-effect waves-light btn-sm" id="import-buttons">批量导入</button>
                            </shiro:hasPermission>
                            <shiro:hasPermission name="customer_exportCustomer">
...
<jsp:include page="/WEB-INF/views/modals/3011.jsp" />
...
$module.customer.getCusOrSurpSelectBean.call($('select[name="id"]')[0],'1',null);
    $(document).ready(function() {
        ...
        $mytable.params={
            ...
            {data:'code'},{data:'name'},{data:'mainId'},{data:'merchantNameStr'},{data:'sourceLabel'},{data:'currencyLabel'},{data:'statusLabel'},{data:'createDate'},
            { //操作
                orderable: false,
                width: "100px",
                data: null,
                render: function (data, type, row, meta) {
                	var status = "";
                	if(row.status == '1'){
                		status = '<shiro:hasPermission name="customer_modifyEnabledCustomer"><a href="javascript:audit('+row.id+',0)">禁用</a></shiro:hasPermission>';
                	}else{
                		status = '<shiro:hasPermission name="customer_modifyEnabledCustomer"><a href="javascript:audit('+row.id+',1)">启用</a></shiro:hasPermission>';
                	}
                	var details = "";
                	details = '<shiro:hasPermission name="customer_toDetailsPage"><a href="javascript:details('+row.id+')">详情</a></shiro:hasPermission>';

                	var rel = "" ;
                	rel = '<shiro:hasPermission name="customer_relMerchant"><br><a href="javascript:relMerchant('+row.id+')">合作公司</a></shiro:hasPermission>';

                	return '<shiro:hasPermission name="customer_toEditPage"><a href="javascript:edit('+row.id+')">编辑</a></shiro:hasPermission>   '+status +'   '+details + rel;
                }
            },
            ],
            formid:'#form1'
    ...
    function edit(id){
    	...
    	$load.a("/customer/toEditPage.html?id="+id);
    }
    ...
    function details(id){
    	...
    	$load.a("/customer/toDetailsPage.html?id="+id);
    }

    //禁用/启用
    function audit(id, status){
    	$custom.alert.warning("你确认要禁用/启用吗？",function(){
	    	var url = "/customer/modifyEnabledCustomer.asyn";
    ...
    function exportLogs(){
    	var data = $("form").serialize();
 		$custom.alert.warning("是否确认导出？",function(){
 		    location.href="/customer/exportCustomer.asyn?" + data;
 		});
    }
