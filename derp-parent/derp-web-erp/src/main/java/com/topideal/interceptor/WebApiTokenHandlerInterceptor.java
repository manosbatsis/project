...
public class WebApiTokenHandlerInterceptor implements HandlerInterceptor {
    private static final Logger logger = Logger.getLogger(WebApiTokenHandlerInterceptor.class) ;
    @Autowired
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if(StringUtils.isBlank(token)){
            logger.info(MessageEnum.LOGINTIMEOUT_99998.getMessage()+"token="+token);
        ...
        if(StringUtils.isBlank(userJson)){
            logger.info(MessageEnum.LOGINTIMEOUT_99998.getMessage()+"2-token="+token);
            ResponseBean responseBean = WebResponseFactory.responseBuild(MessageEnum.LOGINTIMEOUT_99998);
            response.setCharacterEncoding("UTF-8");
            response.setContentType("application/json; charset=utf-8");
            PrintWriter writer = response.getWriter();
