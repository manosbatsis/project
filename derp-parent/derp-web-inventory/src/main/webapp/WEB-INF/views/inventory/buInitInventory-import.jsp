                ...
                <div class="col-12 row">
                    <div>
                       ...
                       <li><i class="dripicons-location"></i> 当前位置：</li>
                        <li class="breadcrumb-item "><a href="#">事业部库存期初列表</a></li>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-responsive table-bordered" cellspacing="0" width="100%">
                        ...
                        <tr>
                            ...
                            <th style="white-space:nowrap; class="tc">仓库自编码</th>
                            <th style="white-space:nowrap;" class="tc">事业部编码</th>
...
$("#btn-file").click(function(){
	...
	$("#form-upload").on("change",'.file-import',function(){
		...
		if(suffix=="xlsx" || suffix=="xls"){
			...
			$custom.request.ajaxUpload(serverAddr+"/buInitInventory/importBuInitInventory.asyn", formData, function(result){
				if(result.state == '200'){
	...
	$("#save-buttons").click(function(){
		var url = serverAddr+"/buInitInventory/saveBuInitInventory.asyn";
		...
		$custom.request.ajax(url, {"jsonObj":jsonObj}, function(result){
			$(".popupbg").hide();
			if(result.state == '200'){
			var success=result.data["success"];//成功条数
			var failure=result.data["failure"];//错误条数
			var msg=result.data["errorMessage"];//提示信息
				if(failure==0){
					$custom.alert.success(msg);	
				}else{
					var messgae="";
					messgae+="成功保存"+success+"条，失败"+failure+"条;"+msg;
	...
	$("#downTemplate").click(function(){
		$downLoad.downLoadByUrl("/excelTemplate/download.asyn?ids=134");
	})
</script>