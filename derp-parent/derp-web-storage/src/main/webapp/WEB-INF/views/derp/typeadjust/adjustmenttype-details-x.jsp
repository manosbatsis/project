                                    ...
                                    <div class="col-7 ml10">
                                    	...
                                    	<%--<c:choose>--%>
                                    		<%--<c:when test="${detail.status eq '020' }">--%>
                                    			<%--待调整--%>
                                    		<%--</c:when>--%>
                                    		<%--<c:when test="${detail.status eq '019' }">--%>
                                    			<%--已调整--%>
                                    		<%--</c:when>--%>
                                    		<%--<c:when test="${detail.status eq '022' }">--%>
                                    			<%--处理中--%>
                                    		<%--</c:when>--%>
                                    	<%--</c:choose>--%>
                                    </div>
                            ...
                            <div class="col-5">
                                <div class="row">
                                    ...
                                    <div class="col-7 ml10">
                                        ${detail.modelLabel}
                                    	<%--<c:choose>--%>
                                    		<%--<c:when test="${detail.model eq '1' }">--%>
                                    			<%--好坏品互转（批次无变更）--%>
                                    		<%--</c:when>--%>
                                    		<%--<c:when test="${detail.model eq '2' }">--%>
                                    			<%--货号变更--%>
                                    		<%--</c:when>--%>
                                    		<%--<c:when test="${detail.model eq '3' }">--%>
                                    			<%--效期调整（批次无变更）--%>
                                    		<%--</c:when>--%>
                                    	<%--</c:choose>--%>
                                    </div>
                        ...
                        <div class="row col-12 mt10">
                            <table id="datatable-buttons" class="table table-striped" cellspacing="0"
                                ...
                                <tr>
                                    ...
                                    <th style="white-space:nowrap;">调整前生产日期</th>
                                    <th style="white-space:nowrap;">调整后生产日期</th>
                                    <th style="white-space:nowrap;">调整前失效日期</th>
                                    <th style="white-space:nowrap;">调整后失效日期</th>
                                ...
                                <c:forEach items="${detail.itemList }" var="item" varStatus="status">
	                                <tr>
	                                    ...
	                                    <td><fmt:formatDate value="${item.oldProductionDate }" pattern="yyyy-MM-dd"/></td>
	                                    <td><fmt:formatDate value="${item.productionDate }" pattern="yyyy-MM-dd"/></td>
	                                    <td><fmt:formatDate value="${item.oldOverdueDate }" pattern="yyyy-MM-dd"/></td>
