        ...
        <div class="modal-dialog  modal-lg" style="max-width:1200px  !important;">
            <div class="modal-content" style="max-width:1200px;">
                ...
                <div class="modal-body">
                    <div class="form-row">
                        <button type="button" class="btn btn-find waves-effect waves-light btn-sm" onclick="$m10012.updateBuName()">分配事业部</button>
                    </div>
                    ...
                    <div class="form-row mt20">
                        <div class="form-inline m0auto">
                            <div class=form-group>
                                <button type="button" class="btn btn-info waves-effect waves-light fr" onclick='$m10012.save();'>确定</button>
                                <button type="button" class="btn btn-light waves-effect waves-light ml15"  onclick='$m10012.hide();'>取消</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<jsp:include page="/WEB-INF/views/modals/10011.jsp" />
<script type="text/javascript">
    var $m10012={
        init:function(id,depotId){
            $m10012.params.inventoryId = id ;
            $m10012.params.depotId = depotId ;
            $m10012.show(id);
            //分配事业部弹窗——事业部下拉框加载
        ...
        show:function(id){
            ...
            $custom.request.ajax($m10012.params.detailUrl, {"id":id}, function(result){
                if(result.state == 200) {
                    ...
                    if(list){
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            var oldGoodType = m.oldGoodType;
                            var newGoodType = m.newGoodType;
                            ...
                            html += '<tr><td class=" td-checkbox"><input type="checkbox"  name="item-checkbox"  value="' + itemId + '" class="iCheck" ' + '></td>'
                                + '<td>' + $m10012.strFormat(goodsNo) + '</td>'
                                + '<td>' + $m10012.strFormat(goodsName) + '</td>'
                                + '<td>' + $m10012.strFormat(barcode) + '</td>'
                                + '<td>' + $m10012.strFormat(buName) + '<input type="hidden" value="' + $m10012.strFormat(buId) + '"></td>'
                                + '<td>' + $m10012.strFormat(oldBatchNo) + '</td>'
                                + '<td>' + $m10012.formatDate(productionDate, "yyyy-MM-dd") + '</td>'
                                + '<td>' + $m10012.formatDate(overdueDate, "yyyy-MM-dd") + '</td>'
                                + '<td>' + $m10012.strFormat(oldGoodType) + '</td>'
                                + '<td>' + $m10012.strFormat(newGoodType) + '</td>'
                                + '<td>' + $m10012.strFormat(adjustTotal) + '</td>'
                                + '<td>' + $m10012.strFormat(tallyingUnitLabel) + '</td>'
                                + '</tr>';
						}
            ...
            $($m10012.params.modalId).modal("show");
        },
        hide:function(){
            $($m10012.params.modalId).modal("hide");
        },
        save:function(){
        ...
        saveData: function () {
            ...
            saveJson.id = $m10012.params.inventoryId;
            ...
            $custom.request.ajax($m10012.params.saveUrl,{"json":jsonStr},function(result){
                if (result.state == 200) {
        ...
        updateBuName:function(){
            ...
            rows = rows.join(",") ;
            $('#modelDepotId').val($m10012.params.depotId);
            //分配事业部弹窗
        ...
        params:{
            detailUrl:serverAddr + '/adjustmentType/toGoodsDetailById.asyn?r='+Math.random(),
            saveUrl:serverAddr + '/adjustmentType/confirmInDepot.asyn?r='+Math.random(),
            inventoryId:"" ,
            depotId:"" ,
